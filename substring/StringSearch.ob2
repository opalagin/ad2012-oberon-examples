...
PROCEDURE Length(src: String): INTEGER;
VAR i: INTEGER;
BEGIN
  i := 0;
  WHILE (i < 255) & (ORD(src[i]) # 0H) DO
    INC(i)
  END;
...
PROCEDURE KMP_IndexOf(p, s: String): INTEGER;
VAR
  i, j, k, N, M: INTEGER;
...
PROCEDURE BM_IndexOf(p, s: String): INTEGER;
VAR
  i, j, k, M, N: INTEGER;
  d: POINTER TO ARRAY OF INTEGER;
BEGIN
